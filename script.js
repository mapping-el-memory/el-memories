...
// Assumes: jQuery, jQuery UI, Leaflet 1.9.x, and js/SliderControl.js are loaded
...
// 2) Data (GeoJSON Feature array); times are YYYY/MM so slider sorts chronologically
var dataset1 = [
...
// 3) Popup templating (template’s optionsObject pattern)
...
// 4) First GeoJSON layer
var group1 = L.geoJSON(dataset1, optionsObject);

// Helper to fit map to a layer/layerGroup if valid
function fitTo(layerOrGroup) {
  try {
    var b = layerOrGroup.getBounds && layerOrGroup.getBounds();
    if (b && b.isValid && b.isValid()) map.fitBounds(b.pad(0.15));
  } catch(e) { /* noop */ }
}

// 5) Load external GeoJSON for a second group, build layerGroup, then slider
$.getJSON("data/popups.json")
  .done(function(json){
    var group2 = L.geoJSON(json, optionsObject);

    // Set the group-level "time" attribute the slider plugin expects
    group1.options.time = "2023";
    group2.options.time = "2024";

    // Combine both groups so the slider shows multiple markers per year
    var multiLayers = L.layerGroup([group1, group2]).addTo(map);

    fitTo(multiLayers);

    // Safety: ensure the plugin is available
    if (!L.control || !L.control.sliderControl) {
      console.error("SliderControl plugin not found. Check js/SliderControl.js include.");
      return;
    }
  ...
  .fail(function(err){
    console.warn("Could not load data/popups.json; using group1 only.", err);
    ...
    // Still give the layer a valid time so the slider renders
    group1.options.time = "2023";
